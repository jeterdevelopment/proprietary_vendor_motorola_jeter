...
set_google_revenue_share_property ()
{
    ...
    if [ $rlz_brandcode ]; then
        case $wave in
            # wave > 2018.4
            2018.4|2019.[1-4] )
                # Google News is not being installed for Comcast and Tracfone, so the Google revenue
                # share property must be set different for these carriers.
                if [ $boot_carrier != 'comcast' ] && [ $boot_carrier != 'tracfone' ]; then
